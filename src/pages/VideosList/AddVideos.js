...
export default function AddVIdeos(props) {
  ...
  const [form, setForm] = React.useState({ id: "", name: "", link: "", src: '' });
  const { id } = useParams();
  ...
  function create() {
    getList();
    id ? setForm({ ...setForm, id: id }) : setForm({ ...setForm, id: list + 1 });
    if (form.name) {
      if (!id) {
        ...
        apiService.update(id, form).then(res => {
          navigate('/videos');
        })
      }
    }
  ...
  function getById() {
    apiService.getById(id).then(res => {
      setForm(res.data);
                ...
                <Item>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setForm({
                        ...form,
                        link: `https://www.youtube.com/watch?v=${video.id.videoId}`, src: `https://www.youtube.com/embed/${video.id.videoId}?autoplay=0&origin=http://example.com&#action=share`
                      })
                    }>
                    Copy url
                  </button>
